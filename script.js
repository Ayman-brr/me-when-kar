const kar = document.querySelector('.kar');
const speedDisplay = document.getElementById('speed-display');

let x = window.innerWidth / 3 - 100; 
let y = window.innerHeight / 2 - 50;
let vx = 0;
let vy = 0; 
let friction = 0.95; // friction factor from physics chapter 3 🙏🏻
let speed = 0; 

let angle = 0; 

const keys = {
  ArrowUp: false,
  ArrowDown: false,
  ArrowLeft: false,
  ArrowRight: false
};

document.addEventListener('keydown', e => { if(keys.hasOwnProperty(e.key)) keys[e.key] = true; });
document.addEventListener('keyup', e => { if(keys.hasOwnProperty(e.key)) keys[e.key] = false; });

function update() {
  const acceleration = 0.5;

  
  if(keys.ArrowLeft) angle -= 3; 
  if(keys.ArrowRight) angle += 3;

  const rad = angle * (Math.PI / 180); // angle to radians

  
  if(keys.ArrowUp) {
    vx += Math.sin(rad) * acceleration;
    vy -= Math.cos(rad) * acceleration; // never thought i'd use sin cos breh
  }
  if(keys.ArrowDown) {
    vx -= Math.sin(rad) * acceleration;
    vy += Math.cos(rad) * acceleration; // +cos means back and vice versa (i think)
  }

  
  vx *= friction;
  vy *= friction;

  
  x += vx;
  y += vy;

  
  speed = Math.sqrt(vx*vx + vy*vy);
  speedDisplay.textContent = 'Speed: ' + Math.round(speed.toFixed(2));

  //how tf am i supposed to find the lower values
  const maxX = window.innerWidth - 50; 
  const maxY = window.innerHeight - 100; 
  if(x < 0) { x=0; vx=0; }
  if(y < 0) { y=0; vy=0; }
  if(x > maxX) { x=maxX; vx=0; }
  if(y > maxY) { y=maxY; vy=0; }

  // apply transform
  kar.style.transform = `translate(${x}px, ${y}px) rotate(${angle}deg)`;

  requestAnimationFrame(update);
}

update();
